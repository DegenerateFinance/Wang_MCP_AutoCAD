...
- **Logging goes to a file via `Log` (`Wang_MCP_AutoCAD/Log.cs`), not `Editor.WriteMessage`.** `Editor` access is only legal on the document thread, so listener threads can't use the AutoCAD console at all; the file log is writable from anywhere. `Log.MinimumLevel` defaults to `Info` and is changeable at runtime with the `MCPLOG` command, which also prints the log path (`%LOCALAPPDATA%\Wang_MCP_AutoCAD\mcp.log`). Keep `WriteMessage` for things a user typing at the command line should see.
- Every MCP `tools/call` handler must marshal onto AutoCAD's document/UI thread before touching the database or editor — use `Application.DocumentManager.MdiActiveDocument` plus its document lock, and scope all database access in a `Transaction` via `using`. No cross-thread AutoCAD database access.
...
- **The listener must be stoppable deterministically, because a live thread pins the collectible `AssemblyLoadContext`.** `IExtensionApplication.Terminate()` has to `Close()` the `HttpListener` (that's what unblocks a thread parked in `GetContext()`) and join the listener thread before returning. Otherwise `Loader.cs`'s `WaitForUnload` spins its ten GC passes, gives up *silently*, and every `MCPRELOAD` leaks a context plus a listener holding the port.
